...
#include "I2Cdev.h" //http://github.com/jrowberg/i2cdevlib
#include "MPU6050.h" //i2cdevlib folder
#include "Wire.h"
//#include "toneAC.h" //https://bitbucket.org/teckel12/arduino-toneac.git
#include "iSin.h"
#include "RunningMedian.h" //https://github.com/RobTillaart/Arduino.git
#include <rtttl.h> //https://github.com/cefn/non-blocking-rtttl-arduino
...
#define NUM_LEDS             150
#define DATA_PIN             3
#define CLOCK_PIN            4 // not used with Neopixels
...
const char pacinter[] PROGMEM = "pacinter:d=4,o=5,b=160:8f#6,8f#6,8f#6,16d#6,16c#6,16f#6,8f#6,8a.6,8p,8f#6,8f#6,8f#6,16d#6,16c#6,16f#6,8f#6,8d#.6,8p,8f#6,8f#6,8f#6,16d#6,16c#6,16f#6,8f#6,8a6,8b6,8c7,8b6,8a6,8f#6,8a.6,8f#6,8p,8f#6,8f#6,8f#6,16d#6,16c#6,16f#6,8f#6,8a.6,8p,8f#6,8f#6,8f#6,16d#6,16c#6,16f#6,8f#6,8d#.6,8p,8f#6,8f#6,8f#6,16d#6,16c#6,16f#6,8f#6,8a6,8b6,8c7,8b6,8a6,8f#6,8a.6,8f#6";
...
const char death[] PROGMEM = "mbdeath:d=4,o=4,b=90:32c6,32c6,32c6,8p,16b,16f6,16p,16f6,16f.6,16e.6,16d6,16c6,16p,16e,16p,16c";
ProgmemPlayer tunePlayer(12);

// POOLS
int lifeLEDs[3] = {6, 7, 8};
Enemy enemyPool[10] = {
...
void setup() {
  ...
  // MPU
  //SerialUSB.println("Starting i2c");
  Wire.begin();
  //SerialUSB.println("Starting MPU6050");
  accelgyro.initialize();

  // Fast LED
  SerialUSB.println("Starting FastLED");
  ...
  SerialUSB.println("Intro...");
  runIntro();
  SerialUSB.println("Loading Level");
  loadLevel();
  SerialUSB.println("Starting game...");
  tunePlayer.setSong(pacman);
}
...
void levelComplete() {
  stageStartTime = millis();
  stage = "WIN";
  tunePlayer.setSong(pacman); //rewind song
...
void SFXtilt(int amount) {
  int f = 0;
  //int f = map(abs(amount), 0, 90, 0, 700);
  //int f = map(abs(amount), 0, 90, 0, 5000);
  //if (playerPositionModifier < 0) f -= 500;
  //if (playerPositionModifier > 0) f += 200;
  if ((millis() % 50) == 0) tone(SPEAKER_PIN, f);
...
void SFXattacking() {
  ...
  if ((millis() % 50) == 0) tone(SPEAKER_PIN, freq);
...
void SFXkill() {
  //if((millis()%50)==0) tone(SPEAKER_PIN, 2000);
...
void SFXwin() {
  ...
  //freq += map(sin(millis() / 20.0) * 1000.0, -1000, 1000, 0, 100);
...
void runIntro() {
  tunePlayer.setSong(galaga);
   static uint8_t hue = 85;
  while (tunePlayer.stepSong()) {
    FastLED.clear();
    FastLED.show();
    for(int i = 0; i < 20; i++){
    leds[map(random8(),0,255,0,NUM_LEDS)] = CHSV(random8(), 255, 150);
    }
    FastLED.show();
    //FastLED.showColor(CHSV(hue=hue+10, 255, 150));
  }
  for (int i = 150; i > 0; i--){
    FastLED.showColor(CHSV(hue, 255, i));
    }
}
